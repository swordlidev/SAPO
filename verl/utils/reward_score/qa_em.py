...
def validate_format(prompt, response):
    ...
    return: (is valid)
    """
    token_list = ['think', 'search', 'memory', 'critic', 'answer']
...
'''
def is_valid_doc_json(s: str) -> bool:
    try:
    ...
    if not isinstance(obj, dict):
        return False
    expected_keys = {"Doc 1", "Doc 2", "Doc 3"}
    if set(obj.keys()) != expected_keys:
        return False
    allowed_values = {"useful", "useless"}
...
def is_valid_doc_json(s: str):
    score = 0
    expected_keys = {"Doc 1", "Doc 2", "Doc 3"}
    sub_expected_keys = {"relevance", "credibility", "summary"}
    allowed_values_r = {"Highly relevant", "Relevant", "Marginally relevant", "Irrelevant"}
    allowed_values_c = {"Credible", "Questionable", "Unreliable", "Low quality"} 
    try:
        obj: Any = json.loads(s)
    except (ValueError, TypeError):
        return  score
    ...
    for k in expected_keys:
        ...
        if not isinstance(sub_obj, dict):
            return  score
        if set(sub_obj.keys()) != sub_expected_keys:
            return  score
        for sub_k in sub_expected_keys:
            v = sub_obj.get(sub_k)
            if sub_k == "relevance" and v not in allowed_values_r:
                return  score
            if sub_k == "credibility" and v not in allowed_values_c:
                return  score
        score += 1
    return  score
...
def validate_critic_format(response):
    """
    validate the template format
    return: (is valid)
    """
    if not response:
        return 0
    score = 0
    info_pattern = r'</documents>\n\n<critic>(.*?)</critic>'
    matches = re.findall(info_pattern, response, re.DOTALL)
    #search_pattern = r'<search>(.*?)</search>\n\n'
    #matches = re.findall(search_pattern, response, re.DOTALL)
    #for imatches in matches:
    #     score += is_valid_doc_json(imatches)
    if len(matches) > 0:
         return is_valid_doc_json( matches[0] )
    return score
    ...
    if answer is None:
        ...
        if answer_score > 0:
            ...
            if refine_score > 0 :
                ...
                if refine_subem > 0:
                    answer_score += refine_score
            if format_score > 0 :
                format_validity = validate_format(solution_str, responses_str)
                if format_validity:
                    answer_score += format_score
                #action_score = format_score if "My previous action is invalid" in responses_str else 0
                critic_score = format_score * validate_critic_format(responses_str) 
                #answer_score -= action_score
                answer_score += critic_score
